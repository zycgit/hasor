...
/** Automatically starts embedded Undertow as part of the Hasor module lifecycle. */
public class UndertowWebServerModule implements Module {
    private UndertowWebServer server;
    ...
    public void loadModule(ApiBinder apiBinder) {
        ...
        if (StringUtils.isNotBlank(serverName) && !"Undertow".equalsIgnoreCase(serverName)) {
            throw new IgnoreModuleException();
        }

        WebServerConfig config = new WebServerConfig().loadSettings(apiBinder.getSettings());
        this.server = new UndertowWebServer(config);
        apiBinder.bindType(WebServer.class).toInstance(this.server);
        apiBinder.bindType(WebServer.class).nameWith("Undertow").toInstance(this.server);
    }
